        ...
        switch (header) {
            case HEADER_solicitud_tabla_segmentos: {
                ...
                if (puedo_crear_proceso(tamanio, memoriaData)) {
                    ...
                    log_info(memoriaData->memoriaLogger, "Se asigno correctamente una tabla de segmentos con ID [%d] y tamaño [%d]", nroTablaNivel1, tamanio);
                } else {
                    stream_send_empty_buffer(socket, HEADER_error);
                    log_error(memoriaData->memoriaLogger, "No se pudo asignar tabla de segmentos con tamaño [%d]", tamanio);
                }
